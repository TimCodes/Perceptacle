...
describe('MongoDBService', () => {
  ...
  describe('Health Check', () => {
    ...
    it('should return unhealthy status on error', async () => {
      ...
      const newService = MongoDBService.fromCredentials(credentials);
      const mockClientInstances = (MongoClient as jest.MockedClass<typeof MongoClient>).mock.instances;
      const latestMockClient = mockClientInstances[mockClientInstances.length - 1];
      
      (latestMockClient.db as jest.Mock).mockReturnValueOnce({
        admin: jest.fn().mockReturnValue({
  ...
  describe('Error Handling', () => {
    it('should throw error when performing operations without connection', async () => {
      ...
      // Mock connection to fail - get the latest instance
      const mockClientInstances = (MongoClient as jest.MockedClass<typeof MongoClient>).mock.instances;
      const latestMockClient = mockClientInstances[mockClientInstances.length - 1];
      (latestMockClient.connect as jest.Mock).mockRejectedValueOnce(new Error('Connection failed'));
